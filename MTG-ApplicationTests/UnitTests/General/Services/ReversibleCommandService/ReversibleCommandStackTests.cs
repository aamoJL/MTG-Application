...
namespace MTGApplicationTests.UnitTests.General.Services.ReversibleCommandService;
...
public class ReversibleCommandStackTests
{
  ...
  [TestMethod(DisplayName = "Redo stack should be cleared when pushing new command to the undo stack")]
  ...
  public void Clear()
  {
    ...
    Assert.IsFalse(stack.CanUndo);
    Assert.IsFalse(stack.CanRedo);
    Assert.IsEmpty(stack.ActiveCombinedCommand.Commands);
  }
}
