...
public class DeckDTORepositoryTests
{
  ...
  public async Task Get_WithName()
  {
    ...
    Assert.HasCount(_savedDeck.DeckCards.Count, result.DeckCards, "Deck card counts don't not match");
    Assert.HasCount(_savedDeck.MaybelistCards.Count, result.MaybelistCards, "Maybelist card counts don't not match");
    Assert.HasCount(_savedDeck.WishlistCards.Count, result.WishlistCards, "Wishlist card counts don't not match");
    Assert.HasCount(_savedDeck.RemovelistCards.Count, result.RemovelistCards, "Remove card counts don't not match");
  }
