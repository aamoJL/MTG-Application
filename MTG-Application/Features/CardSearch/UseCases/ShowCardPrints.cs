  ...
  public class ShowCardPrints(CardSearchViewModel viewmodel) : ViewModelAsyncCommand<CardSearchViewModel, MTGCard>(viewmodel)
  {
    protected override async Task Execute(MTGCard? card)
    {
      try
      {
        ...
        var prints = (await (Viewmodel as IWorker).DoWork(Viewmodel.Importer.ImportWithUri(pageUri: card.Info.PrintSearchUri, paperOnly: true, fetchAll: true))).Found.Select(x => new MTGCard(x.Info));
