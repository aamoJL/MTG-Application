...
public partial class AdvancedCardListView : ListView
{
  public AdvancedCardListView()
  {
    DragAndDrop = new(itemToArgsConverter: (item) => new CardMoveArgs(item, item.Count))
    {
      ...
      OnBeginMoveTo = async (item) => await (OnDropBeginMoveTo?.ExecuteAsync(new DeckEditorMTGCard(item.Card.Info) { Count = item.Count }) ?? Task.CompletedTask),
      OnBeginMoveFrom = (item) => OnDropBeginMoveFrom?.Execute(new DeckEditorMTGCard(item.Card.Info) { Count = item.Count }),
      OnExecuteMove = (item) => OnDropExecuteMove?.Execute(new DeckEditorMTGCard(item.Card.Info) { Count = item.Count })
    };
