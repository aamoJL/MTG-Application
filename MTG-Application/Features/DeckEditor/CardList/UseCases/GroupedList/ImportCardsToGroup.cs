      ...
      try
      {
        ...
        if (result.Source == CardImportResult.ImportSource.External)
        {
          if (result.Found.Length != 0 && result.NotFoundCount == 0) // All found
            new ShowNotification(Viewmodel.Notifier).Execute(new(NotificationType.Success,
            ...
            new ShowNotification(Viewmodel.Notifier).Execute(new(NotificationType.Warning,
            ...
            new ShowNotification(Viewmodel.Notifier).Execute(new(NotificationType.Error, $"Error. No cards were imported."));
        }
      }
      catch (Exception e)
      {
