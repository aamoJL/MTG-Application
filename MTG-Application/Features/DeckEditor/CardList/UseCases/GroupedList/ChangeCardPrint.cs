...
{
  public class ChangeCardPrint(CardGroupViewModel viewmodel) : ViewModelAsyncCommand<CardGroupViewModel, DeckEditorMTGCard>(viewmodel)
  {
        ...
        try
        {
          ...
          if (await Viewmodel.Confirmers.ChangeCardPrintConfirmer.Confirm(CardListConfirmers.GetChangeCardPrintConfirmation(prints.Select(x => new MTGCard(x)))) is MTGCard selection)
          {
            ...
            Viewmodel.UndoStack.PushAndExecute(
              ...
              {
                ReversibleAction = new ReversibleCardPrintChangeAction(Viewmodel.Cards)
              });
          }
        }
        catch (Exception e)
