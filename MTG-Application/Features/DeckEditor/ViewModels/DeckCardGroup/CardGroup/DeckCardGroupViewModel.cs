...
public partial class DeckCardGroupViewModel : ViewModelBase
{
  ...
  private async Task AddCard(DeckEditorMTGCard? card)
  {
    try
    {
      ...
      else
      {
        ...
        {
          ReversibleAction = new ReversibleAddCardsToGroupAction(Model)
        });
      }
    }
    catch (Exception e)
  ...
  private void BeginMoveFrom(DeckEditorMTGCard? card)
  {
    ...
    UndoStack.ActiveCombinedCommand.Commands.Add(
      ...
      {
        ReversibleAction = new ReversibleRemoveCardsFromGroupAction(Model)
      });
  }
  ...
  private async Task BeginMoveTo(DeckEditorMTGCard? card)
  {
    ...
    if (Model.GetFromSource(x => x.Info.Name == card.Info.Name) is DeckEditorMTGCard existingCard)
    {
      if (existingCard == card)
      {
        // Card is from the same source
        // Change move action to group change action
        UndoStack.ActiveCombinedCommand.Cancel();

        UndoStack.PushAndExecute(new ReversiblePropertyChangeCommand<DeckEditorMTGCard, string>(existingCard, existingCard.Group, GroupKey)
        {
          ReversibleAction = new ReversibleCardGroupChangeAction()
        });
      }
      ...
      {
        // Card exists in the source, but is from a different source
        // Confirm conflict
        switch (await ListConfirmers.ConfirmAddSingleConflict(Confirmations.GetAddSingleConflictConfirmation(card.Info.Name)))
        {
    ...
    {
      // Card not in the source, add to the group
      combinedCommands.Add(new ReversibleCollectionCommand<DeckEditorMTGCard>(card)
      {
        ReversibleAction = new ReversibleAddCardsToGroupAction(Model)
      });
    }
  ...
  private void DeleteGroup()
  {
    try
    {
      ...
      new ShowNotification(Notifier).Execute(new(NotificationType.Success, "Group removed successfully."));
    }
    catch (Exception e)
    {
      new ShowNotification(Notifier).Execute(new(NotificationType.Error, e.Message));
  ...
  private async Task RenameGroup()
  {
    try
    {
      ...
      if (newKey == GroupKey)
        return; // Cancel, same key

      if (!NameValidator(newKey))
      {
        // Merge confirmation
        switch (await Confirmers.ConfirmMergeGroups(GroupConfirmations.GetMergeCardGroupsConfirmation(newKey)))
        {
          case ConfirmationResult.Yes: break;
          default: return; // Cancel
        }
      ...
      UndoStack.PushAndExecute(
        new ReversiblePropertyChangeCommand<DeckEditorCardGroup, string>(Model, GroupKey, newKey)
        {
