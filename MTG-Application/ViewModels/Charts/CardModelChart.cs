...
namespace MTGApplication.ViewModels.Charts
{
  ...
  public abstract class CardModelChart<TPrimaryType, TModel> where TModel : ObservableObject
  {
    ...
    protected virtual void Models_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
    {
      switch (e.Action)
      {
        case NotifyCollectionChangedAction.Add: AddToChartSeries(e.NewItems[0] as TModel); break;
        case NotifyCollectionChangedAction.Remove: RemoveFromChartSeries(e.OldItems[0] as TModel); break;
      ...
      foreach (var primaryItem in GetPrimaryItems(model))
      {
        ...
        { 
          if (FindSecondaryItem(series, model) is CardModelSeries<TModel> secondaryItem)
          {
            ...
            if (secondaryItem.PrimaryValue == 0) { (series.Values as ObservableCollection<CardModelSeries<TModel>>).Remove(secondaryItem); } // remove secondary item if its count is zero
          }
